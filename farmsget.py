...
total_minted = farm_contract.functions.totalSupply().call()

for i in range(1, total_minted):
    print(i)
    opensea_address, farm_address, farm_id = farm_contract.functions.getFarm(i).call()
    farmers[opensea_address] = farm_id
    farmers[farm_address] = farm_id
    farmers['farms'][str(farm_id)] = {"opensea_address": opensea_address, "farm_address": farm_address}

    with open("erc1155/farmers.json", 'w') as f:
        logger.info('erc1155/farmers.json regenerated from github, writing file.')
        f.write(json.dumps(farmers))
